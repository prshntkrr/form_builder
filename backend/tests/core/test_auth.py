...
@pytest.mark.parametrize("role,expected", [
    (ROLE_FIELD, {form_perms.RECORDS_VIEW, form_perms.RECORDS_CREATE}),
    (ROLE_EDITOR, {form_perms.FORMS_VIEW, form_perms.RESPONSES_VIEW}),
...
def test_may_asks_about_a_permission_not_a_role(account):
    ...
    assert auth_service.may(user, form_perms.RECORDS_VIEW) is True
    assert auth_service.may(user, form_perms.FORMS_VIEW) is False


def test_nobody_and_the_deactivated_hold_nothing(account):
    assert auth_service.may(None, form_perms.RECORDS_VIEW) is False
    user = account(ROLE_ADMIN)
    assert auth_service.may({**user, "is_active": False}, form_perms.RECORDS_VIEW) is False
