...
def test_the_catalogue_is_in_a_stable_order():
    assert permissions.clean([form_perms.FORMS_VIEW, form_perms.RECORDS_VIEW]) == \
           [form_perms.RECORDS_VIEW, form_perms.FORMS_VIEW]
...
def test_creating_a_role(role):
    created = role(
        "Supervisor",
        permission_keys=[form_perms.RECORDS_VIEW, form_perms.RESPONSES_VIEW],
    )
    assert created["name"] == "supervisor"
    assert created["is_system"] is False
    assert created["permissions"] == [form_perms.RECORDS_VIEW, form_perms.RESPONSES_VIEW]
...
def test_changing_the_permissions(role):
    created = role("Changeable", permission_keys=[form_perms.RECORDS_VIEW])
    updated = role_service.update_role(
        created["role_id"], permission_keys=[form_perms.FORMS_VIEW, form_perms.FORMS_EDIT])
    assert set(updated["permissions"]) == {form_perms.FORMS_VIEW, form_perms.FORMS_EDIT}


def test_omitting_permissions_leaves_them_alone(role):
    created = role("Renamed", permission_keys=[form_perms.RECORDS_VIEW])
    updated = role_service.update_role(created["role_id"], label="Renamed Twice")
    assert updated["label"] == "Renamed Twice"
    assert updated["permissions"] == [form_perms.RECORDS_VIEW]


def test_the_admin_role_cannot_lose_its_keys():
    admin = role_service.get_by_name(ROLE_ADMIN)
    with pytest.raises(role_service.RoleError) as caught:
        role_service.update_role(admin["role_id"], permission_keys=[form_perms.RECORDS_VIEW])
...
def test_a_holder_gets_exactly_the_roles_permissions(holder):
    _, session = holder([form_perms.RECORDS_VIEW, form_perms.FORMS_VIEW])
    assert set(session["user"]["permissions"]) == {
        form_perms.RECORDS_VIEW, form_perms.FORMS_VIEW}


def test_editing_the_role_changes_what_a_holder_may_do(holder):
    created, session = holder([form_perms.RECORDS_VIEW])
    assert auth_service.may(session["user"], form_perms.FORMS_VIEW) is False

    role_service.update_role(
        created["role_id"],
        permission_keys=[form_perms.RECORDS_VIEW, form_perms.FORMS_VIEW])
...
# --- modules switched off in .env ------------------------------------------ #
def test_editing_a_role_keeps_permissions_this_deployment_cannot_see(role):
    """A module switched off must not cost a role the permissions it held.

    The screen cannot show a key the catalogue no longer contains, so it cannot
    send it back either. Dropping it would make switching a module off a
    one-way door.
    """
    created = role("Holds Something Unseen", permission_keys=[permissions.USERS_MANAGE])

    with transaction() as cur:
        cur.execute(
            "INSERT INTO role_permission (role_id, permission) VALUES (%s, %s)",
            (created["role_id"], "notinstalled.view"),
        )

    role_service.update_role(created["role_id"], permission_keys=[permissions.ROLES_MANAGE])

    with transaction() as cur:
        cur.execute("SELECT permission FROM role_permission WHERE role_id = %s",
                    (created["role_id"],))
        stored = {r["permission"] for r in cur.fetchall()}

    assert "notinstalled.view" in stored, "an unseen permission was silently dropped"
    assert permissions.ROLES_MANAGE in stored
    assert permissions.USERS_MANAGE not in stored, "the visible ones still get replaced"
