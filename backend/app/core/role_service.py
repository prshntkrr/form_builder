...
def _set_permissions(cur, role_id: str, keys: Sequence[str]) -> List[str]:
    """Replace a role's permissions, keeping any this deployment cannot see.

    A module switched off in .env contributes nothing to the catalogue, so the
    screen that assigns permissions cannot show its keys and cannot send them
    back. Deleting them on that basis would quietly strip a role of permissions
    nobody chose to remove, and switching the module back on would not restore
    them. So rows for keys outside the current catalogue are left alone: turning
    a module off stays reversible.
    """
    cur.execute("SELECT permission FROM role_permission WHERE role_id = %s", (role_id,))
    invisible = [r["permission"] for r in cur.fetchall()
                 if r["permission"] not in permissions.ALL]

    wanted = permissions.clean(keys)
    cur.execute("DELETE FROM role_permission WHERE role_id = %s", (role_id,))
    for key in wanted + invisible:
        cur.execute(
